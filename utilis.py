...
#'USER DATA' -    #Upload Dataframe to table "dim_users" in the database                    
 ...
 #'SRORE DATA' - #Upload Dataframe to table table "dim_store_details" in the database
    ...
    #connector.upload_to_db(df_store, "dim_store_details", cleaned_file_path) 
    #print("Sucessfuly uploaded 'dim_users' to the database")    


 # 'CARD DATA' #Upload Dataframe to table table "dim_card_details" in the database 
    #raw_creds = connector.read_db_creds(raw_file_path)
    #cleaned_creds = connector.read_db_creds(cleaned_file_path)
    #tables = connector.list_db_tables(raw_file_path)
    #print(tables)
 
 #'SRORE PRODUCTS' - #Upload Dataframe to table table "dim_products" in the database
    #df_store = cleaner.convert_product_weights(raw_file_path)
    #connector.upload_to_db(df_store, "dim_products", cleaned_file_path) 
    #print("Sucessfuly uploaded 'dim_products' to the database") 

 #'ORDERS' - #Upload Dataframe to table table "orders_table" in the database
    df_orders = cleaner.clean_orders_data(raw_file_path)
    connector.upload_to_db(df_orders, "orders_table", cleaned_file_path)
    print("Sucessfuly uploaded 'orders_table' to the database") 
